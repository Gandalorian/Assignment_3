...
void drawDebugging(){
    ...
    text("Actions taken: " + qLearning.actionsTaken, gridSize * cellSize + 20, height - 120);
...
void drawDebugNode() {
    ...
    text(nf(node.weights[0], 3, 3), x + 80, y + 15); // Up
    text(nf(node.weights[3], 3, 3), x + 145, y + 105); // Right
    text(nf(node.weights[1], 3, 3), x + 80, y + 190); // Down
    text(nf(node.weights[2], 3, 3), x + 5, y + 105); // Left
